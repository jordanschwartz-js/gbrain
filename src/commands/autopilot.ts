...
export async function runAutopilot(engine: BrainEngine, args: string[]) {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      'Usage: gbrain autopilot [--repo <path>] [--interval N] [--json] [--no-pull]\n' +
  ...
  console.log(`Autopilot starting. Repo: ${repoPath}, interval: ${baseInterval}s, pull: ${pull ? 'on' : 'off'}`);
...
function writeWrapperScript(repoPath: string, opts: { noPull?: boolean } = {}): string {
  const home = process.env.HOME || '';
...
async function installDaemon(engine: BrainEngine, args: string[]) {
  ...
  const wrapperPath = writeWrapperScript(repoPath, { noPull: args.includes('--no-pull') });
  const home = process.env.HOME || '';
