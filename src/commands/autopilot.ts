...
export function resolveAutopilotCycleTimeoutMs(baseIntervalSeconds: number): number {
  const intervalMs = Math.max(1, baseIntervalSeconds) * 1000;
  return Math.max(intervalMs * 4, 30 * 60 * 1000);
}

export function buildAutopilotCycleJobData(repoPath: string, noPull: boolean): { repoPath: string; noPull?: true } {
  return noPull ? { repoPath, noPull: true } : { repoPath };
}

export function buildAutopilotCycleJobOptions(baseIntervalSeconds: number, slot: string) {
  return {
    queue: 'default',
    idempotency_key: `autopilot-cycle:${slot}`,
    max_attempts: 2,
    timeout_ms: resolveAutopilotCycleTimeoutMs(baseIntervalSeconds),
    max_stalled: 20,
    maxWaiting: 1,
  };
}
...
export async function runAutopilot(engine: BrainEngine, args: string[]) {
  if (args.includes('--help') || args.includes('-h')) {
    console.log(
      ...
      '       gbrain autopilot --install [--repo <path>] [--no-pull]\n' +
      '       gbrain autopilot --uninstall\n' +
      '       gbrain autopilot --status [--json]\n\n' +
    ...
    if (useMinionsDispatch) {
      ...
      try {
        ...
        const job = await queue.add('autopilot-cycle',
          buildAutopilotCycleJobData(repoPath, noPull),
          buildAutopilotCycleJobOptions(baseInterval, slot),
        );
        if (jsonMode) {
...
function writeWrapperScript(repoPath: string, noPull = false): string {
  const home = process.env.HOME || '';
...
exec '${safeGbrainPath}' autopilot --repo '${safeRepoPath}'${noPull ? ' --no-pull' : ''}
`;
...
async function installDaemon(engine: BrainEngine, args: string[]) {
  ...
  const wrapperPath = writeWrapperScript(repoPath, args.includes('--no-pull'));
  const home = process.env.HOME || '';
