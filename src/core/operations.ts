...
const put_page: Operation = {
  ...
  handler: async (ctx, p) => {
    ...
    // Skip embedding when the active provider is not configured. importFromContent's
    // existing try/catch around embed only catches; without a key the OpenAI client would
    // attempt 5 retries with exponential backoff (up to ~2 minutes total) before
    // giving up. Detect early.
    const noEmbed = !isEmbeddingProviderConfigured();
