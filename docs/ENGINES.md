...
**Embedding is NOT in the engine.** The engine stores embeddings and searches by vector, but it doesn't generate embeddings. `src/core/embedding.ts` handles that through the configured provider. This is intentional: embedding is an external model call, not a storage concern. All engines share the same embedding service.
