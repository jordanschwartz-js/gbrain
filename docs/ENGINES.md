...
**Embedding is NOT in the engine.** The engine stores embeddings and searches by vector, but it doesn't generate embeddings. `src/core/embedding.ts` handles the configured embedding provider, such as OpenAI or Ollama. This is intentional: embedding generation is a provider concern, not a storage concern. All engines share the same embedding service.
